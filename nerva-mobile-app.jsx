...
function NervaMobileApp() {
  ...
  const handleScore = useCallback(async () => {
    ...
    // Belt-and-suspenders: offline keyword check first (instant, no network)
    // (already done above via classify() — this is the AI gate)

    setIsScoring(true);
    try {
      // ── AI safety gate — runs BEFORE parse, fails open on network error ──
      try {
        const safeRes = await fetch('/api/safety', {
          method: 'POST',
        ...
        if (safeRes.ok) {
          ...
          if (safeData.category && safeData.category !== 'safe') {
            setSafetyFlag(safeData.category === 'crisis' ? 'crisis' : 'professional');
            setIsScoring(false);
            return;
          }
        }
      } catch (safeErr) {
        // Safety endpoint unavailable — proceed, keyword gate already ran
        console.warn('[nerva] safety check failed, proceeding', safeErr);
      }
